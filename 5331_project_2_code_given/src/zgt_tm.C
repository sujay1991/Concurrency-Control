...
int zgt_tm::TxRead(long tid, long obno, int thrNum)
 {
   ...
   int status;
   status = pthread_create(&threadid[thrNum],NULL,readtx,(void*)nodeinfo);// Fork a thread with tid
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
...
int zgt_tm::AbortTx(long tid, int thrNum)
 {
   ...
   nodeinfo->count = --SEQNUM[tid] ;
   int status;
   status = pthread_create(&threadid[thrNum],NULL,aborttx,(void*)nodeinfo);// Fork a thread with tid
   if (status){
     printf("ERROR: return code from pthread_create() is:%d\n", status);
     exit(-1);
   }
